...
class GLOBAL : NSObject {
    ...
    func showLoadingIndicatorWithMessage(_ message : String){
        ...
        let imageData = NSData(contentsOf: Bundle.main.url(forResource: "animationImg2",withExtension: "gif")!)
    ...
    */
    ...
    func RSVPStatusText(eventStatus : UserEventStatus,userName: String?) -> String? {
        var RSVPStatus : String?
        var suffixStaus : String?
        var prefixStaus : String?
        
        if eventStatus.isUserAdmin {
            if userName != nil {
                prefixStaus = String.init(format: "%@ is ", userName!)
            } else {
                prefixStaus = "You are "
            }
            suffixStaus = "hosting"
            
            if let prefixStaus = prefixStaus, let suffixStaus = suffixStaus {
                RSVPStatus = String.init(format: "   %@%@   ", prefixStaus,suffixStaus)
            }
            else {
                RSVPStatus = nil
                suffixStaus = nil
        ...
        else if eventStatus.isUserArtist {
            if userName != nil {
                prefixStaus = String.init(format: "%@ is ", userName!)
            } else {
                prefixStaus = "You are "
            }
            suffixStaus = "performing"
            if let prefixStaus = prefixStaus, let suffixStaus = suffixStaus {
                RSVPStatus = String.init(format: "   %@%@   ", prefixStaus,suffixStaus)
            }
            else {
            ...
            if let userRSVPStatus = eventStatus.rsvpStatus {
                
                if userRSVPStatus == "invited" {
                    if userName != nil {
                        prefixStaus = String.init(format: "%@ is ", userName!)
                    } else {
                        prefixStaus = "You are "
                ...
                else if userRSVPStatus == "interested" {
                    if userName != nil {
                        prefixStaus = String.init(format: "%@ is ", userName!)
                    } else {
                        prefixStaus = "You are "
                    }
                    suffixStaus = "interested"
                }
                else if userRSVPStatus == "attending" {
                    if userName != nil {
                        prefixStaus = String.init(format: "%@ is ", userName!)
                    } else {
                        prefixStaus = "You are "
                    }
                    suffixStaus = "attending"
                }
                else if userRSVPStatus == "joined" {
                    if userName != nil {
                        prefixStaus = String.init(format: "%@ has ", userName!)
                    } else {
                        prefixStaus = "You have "
                    }
                    suffixStaus = "joined"
                }
                else {
    ...
    func checkFacebookUserEventsUpdated() {
        var userEventsUpdated : Bool = false
        
        UNTZReqeustManager.sharedInstance.checkFacebookUserEvents() {
            (feedResponse) -> Void in
            
            if let downloadError = feedResponse.error{
                print(downloadError)
                userEventsUpdated = false
            } else {
                if let dictionary = feedResponse.responseDict as? Dictionary<String, AnyObject>{
                    if let data = dictionary["data"] as? Dictionary<String, AnyObject> {
                        userEventsUpdated = (data["userEventsUpdated"] as? Bool)!
                    } else {
                        userEventsUpdated = false
                    }
                } else {
