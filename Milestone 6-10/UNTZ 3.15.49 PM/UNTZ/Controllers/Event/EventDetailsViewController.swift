...
class EventDetailsViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func displayUI() {
        ...
        if let distance  = self.eventDetailsResponse?.eventInfo?.distanceFromDeviceLocation {
            self.placeLable.text = String.init(format: "%@ (%0.01f miles)", (self.eventDetailsResponse?.eventInfo?.eventLocation?.locationName)!,distance)
        } else {
            self.placeLable.text = String.init(format: "%@ (0.0 miles)", (self.eventDetailsResponse?.eventInfo?.eventLocation?.locationName)!)
        }
        ...
        var truncatedEventAdminName = String(eventAdminName.characters.dropLast())
        truncatedEventAdminName = String(truncatedEventAdminName.characters.dropLast())
