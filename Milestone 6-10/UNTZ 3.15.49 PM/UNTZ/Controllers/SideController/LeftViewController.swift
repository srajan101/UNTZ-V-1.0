...
class LeftViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if FBSDKAccessToken.current() == nil {
            ...
            if indexPath.row == 0 {
                ...
                viewController.accountId =          (UserInfo.sharedInstance.getUserDefault(key: UNUserInfoKeys.userID) as! String)
        ...
        if((FBSDKAccessToken.current()) != nil){
            FBSDKGraphRequest(graphPath: "me", parameters: ["fields": "id, name, first_name, last_name, picture.type(large), email"]).start(completionHandler: { (connection, result, error) -> Void in
                if (error == nil){
                    ...
                    UserInfo.sharedInstance.setUserInfoInCache(value: uid as AnyObject?, key: UNUserInfoKeys.userID)
 ...
 navigationController.setViewControllers([viewController], animated: false)
    ...
    func registerDeviceToken(){
        ...
        let deviceToken = defaults.object(forKey: CacheConstants.deviceToken)
        if (deviceToken != nil) {
            UNTZReqeustManager.sharedInstance.apiregisterDeviceToken(token: deviceToken as! String) {
